    ...
    if request.cookies:
        if request.method == "POST":
            ...
            if request.form.get("status"):
                ...
                status = data[request.cookies.get("x-session-name")]["profile"][
                    "status"
                ]
            data.update_one(
                {
                    "name": re.compile(
                        request.cookies.get("x-session-name"), re.IGNORECASE
                    )
                },
    ...
    try:
      if request.cookies.get("x-session-name"):
        ...
        if request.method == "POST":
            ...
            if (
                len(request.form.get("uname")) < 2
                or len(request.form.get("uname")) > 30
            ):
                return render_template(
                    "/accounts/signin.html",
